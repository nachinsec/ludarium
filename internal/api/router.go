...
func NewRouter(d Deps) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/igdb/game/{id}", h.igdbGame)
			r.Get("/igdb/popular", h.igdbPopular)
			r.Get("/igdb/upcoming", h.igdbUpcoming)
			r.Get("/showcases", h.getShowcases)
			r.Get("/showcases/{id}", h.getShowcaseGames)
			r.Get("/calendar", h.getCalendar)

			// Conference bingo boards.
			r.Get("/bingo", h.listBingo)
			r.Post("/bingo", h.createBingo)
			r.Get("/bingo/{id}", h.getBingo)
			r.Patch("/bingo/{id}", h.updateBingo)
			r.Delete("/bingo/{id}", h.deleteBingo)
